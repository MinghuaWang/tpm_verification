...
// [start] add by minghua
//#include <tis_builtin.h>
// tis_memcpy -> memcpy, 2 places
// [end]
...
\
    ...
    if (rc) \
        return rc; \
\
    /* Update the size to the real value .    modify by minghua. add (UINT16) at following. */ \
    *(UINT16 *)ptr = HOST_TO_BE_16((UINT16)(buffer + local_offset - ptr - 2)); \
\
    if (offset != NULL) { \
        *offset = local_offset; \
        LOG_DEBUG("offset parameter non-NULL, updated to %zu", *offset); \
