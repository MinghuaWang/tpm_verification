...
*/
int xxx_gcry_md_get_algo_dlen(int algid) ; //{
//     return tis_interval(0, INT_MAX);
// }
//END DEBUG !!!
                            ...
                            TPM2_ALG_ID hashAlg)
{
    ...
    // DEBUG !!!
    int hash_len = xxx_gcry_md_get_algo_dlen(mycontext->hash.gcry_hash_alg);
    //int hash_len = tis_interval(0, INT_MAX);
    // END DEBUG !!!
    if (hash_len <= 0) {
        LOG_ERROR("Unsupported hash algorithm (%"PRIu16")", hashAlg);
        free(mycontext);
    ...
    // unsigned long param_ctx = tis_unsigned_long_interval(0, ULONG_MAX);
    // int param_hash_alg = tis_interval(0, INT_MAX);
    // int param_flag = tis_interval(0, INT_MAX);
    // gcry_error_t r = tis_gcry_md_open(param_ctx, param_hash_alg, param_flag);
    // END DEBUG !!!
    if (r != 0) {
